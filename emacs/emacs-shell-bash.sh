...
# https://github.com/akermu/emacs-libvterm/blob/master/etc/emacs-vterm-bash.sh

# TODO: ansi-osc-directory-tracker uses url encoding to make passing messages more stable
# This can benefit from that too
...
# various symbols.
# export PROMPT_COMMAND="${PROMPT_COMMAND:+$PROMPT_COMMAND; }"'echo -ne "\033]0;${HOSTNAME}:${PWD}\007"'
...
export -f vterm_prompt_command

# export PROMPT_COMMAND="${PROMPT_COMMAND:+$PROMPT_COMMAND; }"'vterm_prompt_command'

"$__tmp_EMACS" --batch --eval '(let ((cl-print-readably t)) (print process-environment))'
